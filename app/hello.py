...
def download():
    ...
    if extention == 'csv':
        return send_file(csv_path, as_attachment=True, attachment_filename=csv_name, mimetype='text/csv')
    elif extention == 'json':
        # CSVファイルの読み込み
        ...
        with open(json_path, 'w') as f:
            json.dump(d_list, f)
        return send_file(json_path, as_attachment=True, attachment_filename=json_name, mimetype='application/json')
...
def read_csv(file_name):
    with open('./data/' + file_name, mode='r', encoding='shift-jis') as f:
